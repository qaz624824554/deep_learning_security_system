...
// 图像后处理，进行物体检测和后续处理
void ImageProcess::image_post_process(cv::Mat &image,
                                    retinaface_result &results, cv::Scalar &color) {
  ...
  for (int i = 0; i < results.count; ++i) {
    ...
    cv::line(image, 
             ...
             cv::Point(detect_result->box.left + FACE_BOX_LENGTH, detect_result->box.top),
             color, 5);
    cv::line(image,
             cv::Point(detect_result->box.left, detect_result->box.top),
             cv::Point(detect_result->box.left, detect_result->box.top + FACE_BOX_LENGTH),
             color, 5);

    // 右上角 
    cv::line(image,
             cv::Point(detect_result->box.right - FACE_BOX_LENGTH, detect_result->box.top),
             cv::Point(detect_result->box.right, detect_result->box.top),
             color, 5);
    cv::line(image,
             cv::Point(detect_result->box.right, detect_result->box.top),
             cv::Point(detect_result->box.right, detect_result->box.top + FACE_BOX_LENGTH),
             color, 5);

    // 左下角
    cv::line(image,
             cv::Point(detect_result->box.left, detect_result->box.bottom - FACE_BOX_LENGTH),
             cv::Point(detect_result->box.left, detect_result->box.bottom),
             color, 5);
    cv::line(image,
             cv::Point(detect_result->box.left, detect_result->box.bottom),
             cv::Point(detect_result->box.left + FACE_BOX_LENGTH, detect_result->box.bottom),
             color, 5);

    // 右下角
    cv::line(image,
             cv::Point(detect_result->box.right - FACE_BOX_LENGTH, detect_result->box.bottom),
             cv::Point(detect_result->box.right, detect_result->box.bottom),
             color, 5);
    cv::line(image,
             cv::Point(detect_result->box.right, detect_result->box.bottom - FACE_BOX_LENGTH),
             cv::Point(detect_result->box.right, detect_result->box.bottom),
             color, 5);

    // char score_text[20];
    // snprintf(score_text, 20, "%0.2f", results.object[i].score);

    // cv::putText(image, score_text,
    //             cv::Point(detect_result->box.left, detect_result->box.top + 20),
    //             cv::FONT_HERSHEY_COMPLEX, 2.5, cv::Scalar(255, 0, 0), 4,
    //             cv::LINE_8); // 绘制类别标签
  }
}
